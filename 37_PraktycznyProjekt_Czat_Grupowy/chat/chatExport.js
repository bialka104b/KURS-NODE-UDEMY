init = (io)=> {//funkcja przyjmuje parametr io dopiero gdy znajdzie się w pliku index.js
    io.on("connection", (socket)=>{//socket to jest nasz klient
        socket.on("join", (nick)=>{//nazwa zdarzenia dowolna ale taka sama jak w pliku chat
            socket.nick = nick; //przypisujemy do socketa(klienta) nick
            io.emit("status", {
                //wyślemy wiadomość o tym że klient się dzalogował do wszystkich innych zalogowanych socketów(klientów)
                //nazwa zdarzenia status jest dowolna
